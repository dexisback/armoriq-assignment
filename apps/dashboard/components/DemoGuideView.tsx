  ...
  const capabilities = [
    {
      ...
      description: "Intercepts incoming tool requests before execution and scans for dangerous command structures.",
      icon: Shield,
    },
    {
      title: "Risk-based Analysis",
      description: "Assigns dynamic risk factors to custom commands to ensure high-risk tasks request manual authorization.",
      icon: Cpu,
    },
    {
      title: "Prompt Security Playground",
      description: "Dedicated playground containing injection scanning heuristics and threat analysis simulation.",
      icon: ShieldAlert,
    },
  ];
  ...
  return (
    <div className="space-y-8">
      <div className="grid grid-cols-12 gap-6">
        ...
        >
          <div>
            <span className="text-[9px] font-mono font-bold uppercase tracking-wider text-accent">Platform Engine</span>
            <h3 className="text-lg font-bold text-foreground mt-1">Capabilities Overview</h3>
            <p className="text-xs text-muted-foreground mt-2 leading-relaxed">
              Armoriq serves as a model-agnostic security plane for agents. It sits directly in the tool loop, evaluating execution safety.
            </p>
          </div>
        ...
        >
          <div>
            <span className="text-[9px] font-mono font-bold uppercase tracking-wider text-accent">System Scope</span>
            <h3 className="text-lg font-bold text-foreground mt-1">Assignment Coverage</h3>
          </div>
          <div className="space-y-2">
            {coverage.map((item, idx) => (
              <div key={idx} className="p-3 bg-muted/30 border border-border/60 rounded-xl flex items-center justify-between gap-4">
                <div className="flex items-center gap-2.5">
                  <CheckCircle2 size={12} className="text-accent shrink-0" />
                  <span className="text-xs font-semibold text-foreground truncate">{item.label}</span>
                </div>
                <span className="text-[9px] font-mono font-medium text-muted-foreground shrink-0">{item.status}</span>
              </div>
            ))}
          </div>
        </motion.div>
        ...
        >
          ...
          <h3 className="text-lg font-bold text-foreground mt-1 mb-6">How Armoriq Works</h3>
